...
export default function OpenTickets() {
  ...
  useEffect(() => {
    const fetchTickets = async () => {
      try {
        let endpoint = "/api/ticket/getclosedtickets";
  ...
  const handleShowMore = async () => {
    try {
      const res = await axios.get(`/api/ticket/getclosedtickets?startIndex=${tickets.length}`);
      const data = res.data;
    ...
    <Flex width="100vw" justifyContent="center" mt="20" mb="10">
      <Box borderWidth="1px" rounded="lg" p={5} width="90%" height="65vh" overflowY="auto" boxShadow="0 5px 10px 0 rgba(158, 158, 158, 0.75)">
        {tickets.length > 0 ? (
          ...
          <Text>No closed tickets found.</Text>
        )}
